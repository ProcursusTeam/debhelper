...
The B<install_dir> function from the perl module B<Debian::Debhelper::Dh_Lib> no longer
attempts to detect "misuse" and will always attempt to chown the created directories
unless debhelper is run in "rootless" mode (per B<Rules-Requires-Root>).  This may
cause some third-party debhelper tools to fail with an error that they cannot
chown a directory due to lack of permissions.  In compat 14 and earlier, this flow
would have triggered a warning since debhelper 13.11.
